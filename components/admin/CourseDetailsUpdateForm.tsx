...
import { Save, X, Upload, Calendar, DollarSign, BookOpen, Users, Award } from 'lucide-react';
import { CourseData, LearningOutcome } from '@/types/course';

interface CourseDetailsUpdateFormProps {
  courseSlug: string;
...
export default function CourseDetailsUpdateForm({ 
  courseSlug, 
  initialData, 
  onClose, 
  onSuccess 
}: CourseDetailsUpdateFormProps) {
  ...
  const [formData, setFormData] = useState<CourseData>({
    ...
    learning_outcomes: [
      ...
      { title: 'রিয়েল প্রোজেক্ট', subtitle: '১২+ প্রোজেক্ট তৈরি', icon: 'CheckSquare' }
    ]
  });
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-[#0f172a] border border-slate-700 rounded-2xl w-full max-w-6xl max-h-[90vh] overflow-y-auto">
        {/* Header */}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-8">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input
                  ...
                  required
                  className="w-full bg-slate-900 border border-slate-700 rounded-lg px-4 py-3 text-white focus:border-blue-500 transition-colors"
                  placeholder="Enter course title"
                />
              </div>
              
              <div>
                <label className="block text-sm font-medium text-slate-400 mb-2">Course Badge</label>
                <select
                  name="badge"
            ...
            <div>
              <label className="block text-sm font-medium text-slate-400 mb-2">Course Description</label>
              <textarea
                name="description"
          ...
          {/* Actions */}
          <div className="flex items-center justify-end gap-4 pt-8 border-t border-slate-700">
            <button
              type="button"
              onClick={onClose}
              className="px-8 py-3 border border-slate-700 text-white rounded-lg hover:bg-slate-800 transition-colors"
            >
              Cancel
            </button>
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex items-center gap-2 px-8 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-lg transition-colors disabled:opacity-50"
            >
              {loading ? (
                <>
