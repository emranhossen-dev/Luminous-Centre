...
const iconMap: Record<string, LucideIcon> = { TrendingUp, CheckSquare, Users, Award };
    ...
    <div className="relative w-full bg-[#020617] text-white overflow-hidden py-16 lg:py-24">
      ...
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
        ...
        <div className="lg:col-span-7 space-y-8">
          <div className="inline-flex items-center gap-2 bg-blue-500/10 border border-blue-500/20 px-4 py-1.5 rounded-full text-blue-400 text-xs font-black uppercase tracking-widest">
          ...
          <div className="flex flex-wrap items-center gap-8 pt-4">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-slate-500 line-through text-sm font-bold">
                Regular {course.regular_price.toLocaleString()} {course.currency}
              </span>
            </div>
          </div>
        ...
        {/* RIGHT: High-End Media Card */}
        <div className="lg:col-span-5 bg-[#0a0f1e]/80 backdrop-blur-2xl border border-white/10 rounded-[3rem] overflow-hidden shadow-2xl">
              ...
              {course.learning_outcomes.map((item, i) => {
                ...
                return (
                  <div key={i} className="flex flex-col items-center text-center space-y-3 group">
                    <div className="bg-blue-500/10 p-3 rounded-2xl text-blue-500 border border-blue-500/10 group-hover:bg-blue-600 group-hover:text-white transition-colors duration-300">
                      <Icon size={24} />
                    </div>
                    <div>
                      <h4 className="font-bold text-[14px] text-slate-100">{item.title}</h4>
...
const StatItem = ({ icon, label, value }: { icon: React.ReactNode, label: string, value: string | number }) => (
  <div className="bg-white/5 border border-white/5 p-5 rounded-3xl flex flex-col items-center justify-center space-y-1 group hover:bg-white/10 transition-all">
    <div className="text-blue-500 mb-1">{icon}</div>
    <span className="text-2xl font-black">{value}</span>
    <span className="text-[9px] uppercase text-slate-500 font-black tracking-[0.1em]">{label}</span>
  </div>
);
